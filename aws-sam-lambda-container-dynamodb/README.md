...
```bash
imagen$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
imagen$ sam local start-api
imagen$ curl http://localhost:3000/
```
...
```bash
imagen$ sam logs -n HelloWorldFunction --stack-name "imagen" --tail
```
...
```bash
imagen$ pip install pytest pytest-mock --user
imagen$ python -m pytest tests/ -v
```
